...
try:
    ...
    # Auto-detect if running remotely
    if 'CLEARML_WORKER_ID' in os.environ:
        task = Task.current_task()
        if task:
            print("Running on ClearML remote worker")
except ImportError:
    CLEARML_AVAILABLE = False
    print("ClearML not available")
...
class CDPRSimulation:
    def __init__(self, xml_path, output_dir="openvla_trajectories", task=None):
        self.xml_path = xml_path
        self.output_dir = output_dir
        self.model = None
        self.data = None
        self.task = task  # ClearML task object
        ...
        # Report to ClearML if available
        if self.task:
            self.task.connect({
    ...
    def run_openvla_trajectory(self, max_steps=1000, vla_update_interval=100):
        ...
        for step in range(max_steps):
            ...
            if step % vla_update_interval == 0:
                ...
                if self.task:
                    self.task.get_logger().report_scalar(
                        "Target Position", "X", iteration=step, value=new_target[0]
                    )
                    self.task.get_logger().report_scalar(
                        "Target Position", "Y", iteration=step, value=new_target[1]
                    )
                    self.task.get_logger().report_scalar(
                        "Target Position", "Z", iteration=step, value=new_target[2]
                    )
            ...
            if self.task:
                self.task.get_logger().report_scalar(
                    "Performance", "Position Error", iteration=step, value=error
                )
            
            if self.check_success():
                success_steps += 1
                if success_steps > 10:  # Require consistent success
                    print(f"Target reached in {step} steps!")
                    if self.task:
                        self.task.get_logger().report_scalar(
                            "Performance", "Success Steps", iteration=step, value=step
                        )
                    break
            else:
                success_steps = 0
        ...
        if self.task and self.overview_frames:
            self._upload_to_clearml(trajectory_dir)
    ...
    def _upload_to_clearml(self, trajectory_dir):
        ...
        if os.path.exists(video_path):
            self.task.upload_artifact('trajectory_video', video_path)
        
        # Upload data
        data_path = os.path.join(trajectory_dir, "trajectory_data.npz")
        if os.path.exists(data_path):
            self.task.upload_artifact('trajectory_data', data_path)
        
        # Report some sample frames
        for i, frame in enumerate(self.overview_frames[::50]):  # Every 50th frame
            self.task.get_logger().report_image(
                "Trajectory Frames", f"frame_{i*50}", iteration=i*50, image=frame
            )
...
def main():
    ...
    if CLEARML_AVAILABLE:
        task = Task.init(
            project_name="CDPR-OpenVLA",
            task_name="CDPR Simulation with OpenVLA",
            task_type=Task.TaskTypes.testing
        )
    
    sim = CDPRSimulation(xml_path, task=task)
    
    try:
        sim.initialize()
        success = sim.run_openvla_trajectory(max_steps=800, vla_update_interval=100)
        
        if success:
            print("✓ OpenVLA trajectory completed!")
            if task:
                task.get_logger().report_text("Simulation completed successfully")
        else:
            print("✗ OpenVLA trajectory failed!")
            if task:
                task.get_logger().report_text("Simulation failed")
            
    except KeyboardInterrupt:
        print("\nTrajectory interrupted by user")
        if task:
            task.get_logger().report_text("Simulation interrupted by user")
    except Exception as e:
